...
import { X, Play, Trash2, Terminal, Activity, Sprout, Hammer, Settings, Key, Dog, Users, ExternalLink, Hand } from 'lucide-react'; // 引入 Hand 图标
...
export function DebugSidebar({ isOpen, onClose, currentPlayerId }: DebugSidebarProps) {
  ...
  const actions = {
    ...
    customRequest: async () => {
      ...
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}${customEndpoint}`, {
          method: customMethod,
          headers,
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[90%] sm:w-[420px] bg-[#1c1917] border-l-2 border-orange-900/50 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar bg-[#1c1917]">
          ...
          <section>
            ...
            <div className="bg-stone-900/50 p-3 rounded border border-stone-800 space-y-3">
              <div className="space-y-1">
                <label className="text-[10px] text-stone-500 font-mono uppercase">User Action</label>
                <div className="flex gap-2">
                  ...
                  {/* [新增] 跳转按钮 */}
                  <button
                    className={`${btnClass} !py-1 !px-2 bg-blue-900/30 hover:bg-blue-900/50 text-blue-200 border-blue-800`}
                    onClick={() => {
                      if (playerName) {
                        router.push(`/u/${encodeURIComponent(playerName)}`);
                        onClose(); // 选跳转后关闭侧边栏
                      }
                    }}
                    title="Go to Profile"
          ...
          {/* Farming Operations */}
          <section>
            <div className={groupTitleClass}><Sprout className="w-3 h-3" /> Actions (Target: {currentPlayerId ? 'Visitor' : 'Self'})</div>
            ...
            {/* [新增] 偷菜按钮 */}
            <div className="mt-2">
              <button className={`${btnClass} w-full bg-red-900/30 border-red-800 text-red-200 hover:bg-red-900/50`} onClick={actions.steal}>
                <Hand className="w-3 h-3" /> Steal (Visitor Only)
              </button>
            </div>
          </section>
            ...
            {logs.map((log) => (
              <div key={log.id} className="mb-3 border-b border-white/10 pb-2 last:border-0 group">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`font-bold ${log.type === 'success' ? 'text-green-500' :
